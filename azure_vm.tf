...
resource "azurerm_virtual_machine_extension" "jenkins_terraform" {
  ...
  settings = <<SETTINGS
  {
          "fileUris": ["https://raw.githubusercontent.com/bitoneer/terraJenk/master/jenkins-init.sh"],
...
terraform {
  backend "azurerm" {
    storage_account_name = "jenkinsterraformbit"
    container_name       = "jenkins-app-blob"
    key                  = "terraform.tfstate"
    access_key           = "Aw2VZwoVKckETuFk3Bg0Dcf0BYWf9arNTspBVgO9TBPUscuUuVhnVEAhFGuwE9XfffoMLvFyKiiN0iBKdigxtw=="
